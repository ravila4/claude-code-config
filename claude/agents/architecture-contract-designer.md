...
description: Use this agent when you need to create comprehensive architecture proposals with visual diagrams that serve as binding implementation contracts. Creates iterative design documents with Graphviz system diagrams and Mermaid interaction flows, following a formal review-and-approval workflow where approved diagrams become executable protocols for implementing agents. Examples: <example>Context: User wants to add a new feature with clear implementation contract. user: 'I need to add real-time validation to our genomics pipeline with a formal architecture approval process' assistant: 'I'll use the architecture-contract-designer agent to create a comprehensive proposal with visual diagrams that will serve as the implementation contract.' <commentary>The user wants formal architecture design with visual contracts - perfect for architecture-contract-designer.</commentary></example> <example>Context: User wants architecture design that agents must follow. user: 'Design the caching layer architecture and create a protocol that implementing agents must follow' assistant: 'Let me use the architecture-contract-designer agent to create both the architecture proposal and the executable protocol contract.' <commentary>This requires the formal contract workflow with Graphviz protocol diagrams.</commentary></example>
